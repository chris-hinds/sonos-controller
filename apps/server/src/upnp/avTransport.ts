...
import type { RepeatMode } from "@kyuu/shared";
...
function parseDidlMetadata(
  didlXml: string,
): Pick<PositionInfo, "title" | "artist" | "album" | "albumArtURI"> {
  ...
  try {
    ...
    const item =
      parsed?.["DIDL-Lite"]?.item || parsed?.["DIDL-Lite"]?.container;
    ...
    const albumArtURI =
      ...
      item["upnp:albumArtUri"] ||
      (Array.isArray(item["upnp:albumArtURI"])
...
async function getTransportInfo(ip: string): Promise<TransportInfo> {
  ...
  const resp =
    body["u:GetTransportInfoResponse"] ||
    body["GetTransportInfoResponse"] ||
    {};
  return {
...
async function getPositionInfo(ip: string): Promise<PositionInfo> {
  ...
  const body = res?.["s:Envelope"]?.["s:Body"] || res?.Envelope?.Body || {};
  const resp =
    body["u:GetPositionInfoResponse"] || body["GetPositionInfoResponse"] || {};
...
async function getMediaInfo(ip: string): Promise<MediaInfo> {
  const res = await soapCall(ip, SERVICE_PATH, SERVICE_TYPE, "GetMediaInfo", {
    InstanceID: 0,
  });
  const body = res?.["s:Envelope"]?.["s:Body"] || res?.Envelope?.Body || {};
  const resp =
    body["u:GetMediaInfoResponse"] || body["GetMediaInfoResponse"] || {};
  return {
...
async function getTransportSettings(ip: string): Promise<TransportSettings> {
  ...
  const body = res?.["s:Envelope"]?.["s:Body"] || res?.Envelope?.Body || {};
  const resp =
    body["u:GetTransportSettingsResponse"] ||
    body["GetTransportSettingsResponse"] ||
    {};
  ...
  if (playMode === "SHUFFLE") {
    shuffle = true;
    repeat = "all";
  } else if (playMode === "SHUFFLE_REPEAT_ONE") {
    shuffle = true;
    repeat = "one";
...
async function play(ip: string): Promise<void> {
  await soapCall(ip, SERVICE_PATH, SERVICE_TYPE, "Play", {
    InstanceID: 0,
    Speed: 1,
  });
}
...
async function removeAllTracksFromQueue(ip: string): Promise<void> {
  await soapCall(ip, SERVICE_PATH, SERVICE_TYPE, "RemoveAllTracksFromQueue", {
    InstanceID: 0,
  });
}

async function addURIToQueue(
  ip: string,
  uri: string,
  metadata = "",
): Promise<void> {
