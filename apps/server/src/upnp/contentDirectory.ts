...
function parseDidlList(resultXml: string): any[] {
  ...
  try {
    ...
    console.error(
      "[ContentDirectory] Error parsing DIDL:",
...
function extractRawItemMetadata(resultXml: string): string[] {
  ...
  const decoded = decodeXmlEntities(resultXml).replace(
    /&(?![a-zA-Z#][a-zA-Z0-9]*;)/g,
    "&amp;",
  );
...
async function getFavorites(ip: string): Promise<FavoriteItem[]> {
  try {
    ...
    console.error(
      `[ContentDirectory] getFavorites error for ${ip}:`,
      (err as Error).message,
    );
    return [];
  }
...
async function getQueue(ip: string): Promise<QueueItem[]> {
  try {
    ...
    console.error(
      `[ContentDirectory] getQueue error for ${ip}:`,
      (err as Error).message,
    );
    return [];
  }
...
async function getSonosPlaylists(ip: string): Promise<ContentItem[]> {
  try {
    ...
    console.error(
      `[ContentDirectory] getSonosPlaylists error for ${ip}:`,
      (err as Error).message,
    );
    return [];
  }
...
async function getAudioInputs(ip: string): Promise<AudioInput[]> {
  try {
    ...
    if (!(err as Error).message.includes("701")) {
      console.error(
        `[ContentDirectory] getAudioInputs error for ${ip}:`,
        (err as Error).message,
      );
    }
    return [];
