...
function broadcastOptimisticState(
  ip: string,
...
router.post("/:ip/pause", async (req, res) => {
  try {
    ...
    const groupId = getGroupId(req.params.ip);
    setGracePeriod(groupId, "transportState", 5000);
    broadcastOptimisticState(req.params.ip, {
...
router.post("/:ip/seek", async (req, res) => {
  try {
    ...
    if (seconds === undefined) {
      res.status(400).json({ error: "seconds required" });
      return;
    }
    const coordIp = getCoordinatorIp(req.params.ip);
        ...
        inputs = [
          {
            id: "tv",
            title: "TV",
            uri: `x-sonos-htastream:${speaker.uuid}:spdif`,
          },
        ];
      }
    }
